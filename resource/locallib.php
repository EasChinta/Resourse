...
function resource_get_file_details($resource, $cm) {
    $options = empty($resource->displayoptions) ? array() : @unserialize($resource->displayoptions);
    $filedetails = array();
    if (!empty($options['showsize']) || !empty($options['showtype']) || !empty($options['showdate']) || !empty($options['showuser'])) {
        $context = context_module::instance($cm->id);
        $fs = get_file_storage();
        ...
        if (!empty($options['showuser'])) {
          ...
          $filedetails['user'] = $file->get_userid();
